...
export default function ProductPage({ params }: Props) {
      ...
      if (jsonStoredData[`${storage?.name}`]) {
        ...
        categories.forEach((category) => {
          Object.keys(category.complements).forEach((complementId) => {
            const index = Number(complementId); // ✅ Converte para número
            if (!isNaN(index)) { // ✅ Garante que a conversão foi bem-sucedida
              restoredQuantities[`${category.id}-${complementId}`] = {
                qtd: Number(category.complements[index]), // ✅ Agora funciona
  ...
  }, [storage?.name, restaurant]); // ✅ Agora `restaurant` está listado corretamente

  // 🚀 Conversão para garantir compatibilidade com ICart
  const cart: Record<string, ICart> = Object.entries(selectedQuantities).reduce(
    (acc, [key, item]) => ({
      ...acc,
      [key]: {
        id: item.complementId, // ✅ Adicionando `id`
        name: item.name, // ✅ Defina um valor adequado para `name`
        qtd: item.qtd,
        limit: item.limit, // ✅ Defina um valor adequado para `limit`
        price: item.price,
        categoryId: item.categoryId,
        complementId: item.complementId,
        observation: item.limit, // ✅ Defina um valor adequado para `observation`
        complements: [], // ✅ Evita erro ao passar um array vazio
      },
    }),
    {}
  );
